...
const Home = () => {
  ...
  const roofingProducts = products.filter(p => p.category?.toLowerCase() === 'roofing').slice(0, 4);
  const topSellingProducts = products.filter(p => p.featured || p.bestSeller).slice(0, 4);
  const accessoriesProducts = products.filter(p => p.category?.toLowerCase() === 'accessories').slice(0, 4);
    ...
    <div>
      ...
      <section id="accessories-products" className="roofing-products">
        ...
        {loading ? (
          ...
          <p style={{ textAlign: 'center' }}>No accessories products available</p>
        )}
      </section>
